...
@onready var CAMERA_FRAME: Sprite2D = $Camera2D/CameraFrame
@onready var CAMERA_IMG: TextureRect = $Camera2D/HandGesture
@onready var SCORE_PANEL: Panel = $Camera2D/ScorePanel
@onready var SCORE_LABEL: Label = $Camera2D/ScorePanel/ScoreLabel
...
func handle_actionables(delta: float) -> void:
	var actionables = ACTIONABLE_FINDER.get_overlapping_areas()
	if actionables.size() > 0:
		match actionables[0].get_character():
			"guard":
				handle_guard_dialogue(actionables[0], delta)
			"wizard":
				handle_wizard_dialogue(actionables[0], delta)
			_:
				actionables[0].action()
				

func handle_guard_dialogue(act, delta) -> void:
	if GuardDialogueState.guard_finished == true:
		CAMERA_FRAME.visible = false
		CAMERA_IMG.visible = false
		SCORE_PANEL.visible = false
		SCORE_LABEL.visible = false
		GuardDialogueState.player_score = 0
		GuardDialogueState.guard_score = 0
		act.deactivate_colliders()
		return
	if GuardDialogueState.guard_started == false:
		act.action()
	CAMERA_FRAME.visible = true
	CAMERA_IMG.visible = true
	SCORE_PANEL.visible = true
	SCORE_LABEL.visible = true
	SCORE_LABEL.text = "Score\n" + str(GuardDialogueState.player_score) + " - " + str(GuardDialogueState.guard_score)
	handle_velocity(2*delta, Vector2.ZERO)	
	
	
func handle_wizard_dialogue(act, delta) -> void:
	if PLAYER_SPRITE.flip_h == true: 
		return
	if WizardDialogueState.wizard_finished == true:
		act.deactivate_colliders()
		return
	if WizardDialogueState.wizard_started == false:
		act.action()
	handle_velocity(2*delta, Vector2.ZERO)
